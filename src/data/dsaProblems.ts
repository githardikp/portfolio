...
export const dsaProblems: DSAProblem[] = [
  {
    ...
    description:
      'A binary search tree maintains an ordered structure that allows O(log n) average-time searches, insertions and deletions. By cataloguing each book by title (or Dewey decimal number) inside a BST, we minimise the number of shelves we must scan when locating a book.',
    complexity: 'Average O(log n) · Worst O(n) (unbalanced)',
    steps: [
  ...
  },
  {
    ...
    description:
      'A k-d tree indexes geographic coordinates enabling efficient nearest-neighbour queries. Once candidate drivers are retrieved, a priority queue (min-heap) is used to continually pop the closest driver as traffic positions update.',
    complexity: 'k-d tree search O(log n) · Heap pop O(log k)',
    steps: [
  ...
  },
  {
    ...
    description:
      'Two stacks (undo & redo) or a doubly linked list with a cursor can capture the history of actions. Each new action pushes state onto the undo stack and clears the redo stack. Moving the cursor backwards/forwards replays history in O(1).',
